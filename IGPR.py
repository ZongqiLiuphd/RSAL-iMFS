...
class IGPR(object):
    def __init__(self, init_x, init_y,theta_f, len,theta_n ):
        self.hyperparam = HyperParam(1, len, theta_n)
        self.max_k_matrix_size = 4000000
        self.lamda = 1
        self.count = 0
        self.kernel_x = deque()
        self.kernel_y = deque()
    ...
    def predict(self, coming_x):
        if self.is_available():
            ...
            cov = 1 - cross_kernel_k.dot(self.inv_k_matrix).dot(cross_kernel_k.T)
        else:
